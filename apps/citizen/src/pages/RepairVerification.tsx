...
export function RepairVerification() {
  ...
  const [issue, setIssue] = useState<Issue | null>(
    isSupabaseConfigured ? null : fixtureIssues.find((i) => i.id === id) ?? fixtureIssues.find((i) => i.status === 'AWAITING_VERIFICATION')!,
  );
  ...
  const composer = (
    <div className="safe-bottom flex items-center gap-2.5 border-t border-border-strong bg-paper-chip px-4 pb-4 pt-3">
      ...
      <button
        ...
        title={isSupabaseConfigured ? 'Comment posting on this screen is not wired up yet' : undefined}
        className="min-h-[48px] rounded-full bg-resolved px-5 font-mono text-[12.5px] font-bold text-paper disabled:opacity-40"
      >
        Verify
      </button>
    </div>
      ...
      <div className="px-4 pb-6 pt-3.5">
        ...
        <div className="rounded-card border border-gov-border bg-gov-bg p-3.5">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="font-mono text-xs font-bold text-gov">{isSupabaseConfigured ? issue.category : 'Roads Department'}</div>
              <div className="font-mono text-[11px] text-muted">{isSupabaseConfigured ? 'awaiting community verification' : '18 Aug · repair submitted'}</div>
            </div>
          </div>
          {isSupabaseConfigured ? (
            <p className="mt-2.5 text-sm leading-relaxed text-gov-text">{issue.body}</p>
          ) : (
            <p className="mt-2.5 text-sm leading-relaxed text-gov-text">Patch completed this morning. 40 sq.m resurfaced and compacted.</p>
          )}
          {!isSupabaseConfigured && (
            <>
        ...
        <div className="mt-3.5 rounded-card border border-resolved-border bg-resolved-bg p-4">
          ...
          {isSupabaseConfigured ? (
            <p className="mt-1.5 text-[13.5px] leading-relaxed text-muted-strong">
              Verification submission isn't wired up on this build yet — the table and RLS policy exist, but there's
              no write endpoint for a citizen verdict, so these buttons don't record anything yet.
            </p>
          ) : (
          ...
          <div className="mt-3.5 flex gap-2.5">
            ...
            <Button
              ...
              icon={<XIcon size={17} />}
              disabled={isSupabaseConfigured}
              onClick={() => setVerdict('not-fixed')}
              className="disabled:opacity-40"
            >
              Not fixed
            </Button>
          </div>
          {verdict && !isSupabaseConfigured && (
